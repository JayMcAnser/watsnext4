          ...
          <div class="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-gray-800">
            ...
            <div class="mt-5 flex-1 h-0 overflow-y-auto">
              <nav class="px-2 space-y-1">
                <router-link v-for="item in navigation" :key="item.label" :to="item.href" :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'group flex items-center px-2 py-2 text-base font-medium rounded-md']">
      ...
      <div class="flex-1 flex flex-col min-h-0 bg-gray-800">
        ...
        <div class="flex-1 flex flex-col overflow-y-auto">
          <nav class="flex-1 px-2 py-4 space-y-1">
            <router-link v-for="item in navigation" :key="item.label" :to="item.href" :class="[activeMenu ===  item.name ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'group flex items-center px-2 py-2 text-sm font-medium rounded-md']">
              <component :is="item.icon" :class="[activeMenu ===  item.name ? 'text-gray-300' : 'text-gray-400 group-hover:text-gray-300', 'mr-3 flex-shrink-0 h-6 w-6']" aria-hidden="true" />
              {{ item.label }}
            </router-link>
          </nav>
        </div>
...
<!--
                    <a v-else
                      :href="item.href" :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']">{{ item.label }}
                    </a>
-->
                  </MenuItem>
                </MenuItems>
...
const navigation = [
  { label: 'Dashboard', href: '/', icon: HomeIcon, current: true, name: 'dashboard' },
    ...
    const action = async (actionName) => {
      switch (actionName) {
        case 'logout':
          ...
          break;
        default:
          error(`unknown action${actionlabel}`)
      }
    }

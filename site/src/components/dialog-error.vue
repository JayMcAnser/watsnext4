...
export default {
  ...
  setup(props) {
    ...
    const hasError = computed(() => {
      return store.getters['status/isError'];
    })
    const message = computed(() => {
      return store.getters['status/errorMessage']
    });
    const title = computed(() => {
      let title = store.getters['status/errorTitle']
      if (title) {
        return title
      }
      return 'Error'
    });
    const errorReset = async function() {
      await store.dispatch('status/clear')
    }
    return {
      hasError,
      errorReset,

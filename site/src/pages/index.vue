                  ...
                  <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                    <MenuItems  class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                      <MenuItem  v-for="item in userNavigation" :key="item.name" v-slot="{ active }">
                        <router-link :to="{name: item.route, params: item.params}" :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']">{{ item.name }}</router-link>
                      </MenuItem>
                    </MenuItems>
                  </transition>
                </Menu>
...
const sidebarNavigation = [
  ...
  { name: 'Distribution', href: '#', icon: ViewGridIcon, current: false },
  { name: 'Royalties', href: '#', icon: PhotographIcon, current: true },
  { name: 'Mediakunst.net', href: '#', icon: UserGroupIcon, current: false },
  { name: 'Contact', href: '#', icon: CollectionIcon, current: false },
